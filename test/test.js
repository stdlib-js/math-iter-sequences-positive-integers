...
var iterPositiveIntegersSeq = require( './../lib' );
...
tape( 'main export is a function', function test( t ) {
	...
	t.strictEqual( typeof iterPositiveIntegersSeq, 'function', 'main export is a function' );
	t.end();
});
...
tape( 'the function returns an iterator protocol-compliant object which generates a positive integer sequence', function test( t ) {
	var expected;
	var actual;
	var it;
	var i;

	it = iterPositiveIntegersSeq();
	...
	for ( i = 0; i < 1000; i++ ) {
		...
		t.strictEqual( isPositiveInteger( actual.value ), true, 'returns a positive integer' );
...
tape( 'the function supports limiting the number of iterations', function test( t ) {
	...
	it = iterPositiveIntegersSeq( opts );
	t.strictEqual( it.next.length, 0, 'has zero arity' );
...
tape( 'if an environment supports `Symbol.iterator`, the returned iterator is iterable', function test( t ) {
	var iterPositiveIntegersSeq;
	var it1;
	var it2;
	var i;

	iterPositiveIntegersSeq = proxyquire( './../lib/main.js', {
		'@stdlib/symbol-iterator': '__ITERATOR_SYMBOL__'
	});

	it1 = iterPositiveIntegersSeq();
